...
const exampleQuestions = [
    ...
    "Show me the highest and lowest weekly sales.",
];
...
async function analyze() {
    ...
    try {
        ...
        const response = await analyzeDataset(trimmedQuestion);
...
<template>
    <section class="w-full">
        ...
        <div
            class="rounded-[22px] border border-[#e7e5f2] bg-gradient-to-br from-white to-[#f9f7ff] p-7 shadow-[0_5px_15px_rgba(15,23,42,0.03),0_18px_45px_rgba(79,70,229,0.06)] max-[700px]:p-5"
        >
            <!-- Header -->
            ...
            >
                <div class="flex items-center gap-[13px]">
                    <div
                        class="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#ede9fe] to-[#e0e7ff] text-[#6d28d9]"
                    >
                        <svg
                            class="h-[22px] w-[22px]"
                            viewBox="0 0 24 24"
                    ...
                    <div>
                        <h2
                            class="m-0 text-[21px] font-bold tracking-[-0.4px] text-[#172033]"
                        >
                            Ask Your Data
                        </h2>

                        <p
                            class="mt-1 text-[13px] leading-normal text-[#7a8496]"
                        >
            ...
            <div class="flex flex-col gap-[9px]">
                ...
                <div class="relative">
                    <textarea
                        ...
                        rows="4"
                        :disabled="loading"
                        placeholder="Example: Show the top 5 stores by average weekly sales."
                ...
                <div class="mt-2">
                    <div
                        class="mb-2 text-[10px] font-bold uppercase tracking-[0.45px] text-[#98a2b3]"
                    >
                        Try an example
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            class="rounded-full border border-[#e5e1f4] bg-white px-3 py-[7px] text-[11px] font-medium text-[#667085] transition hover:border-[#c4b5fd] hover:bg-[#f8f5ff] hover:text-[#6d28d9] disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            {{ example }}
                        </button>
                    </div>
                ...
                >
                    ...
                    <div class="flex items-center gap-2 max-[700px]:w-full">
                        <button
                            ...
                            :disabled="loading"
                            @click="clearAnalysis"
                            class="rounded-[11px] border border-[#ddd8ed] bg-white px-[14px] py-3 text-[13px] font-semibold text-[#667085] transition hover:border-[#c9c1df] hover:bg-[#faf9ff] disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            Clear
                        </button>
        ...
        <div
            v-if="loading"
            class="mt-[18px] flex items-center gap-4 rounded-[14px] border border-[#e7e5f2] bg-white px-5 py-4 shadow-[0_5px_15px_rgba(15,23,42,0.03)]"
        >
            <div
                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f3efff]"
            >
                <span
                    class="h-4 w-4 animate-spin rounded-full border-2 border-[#ddd6fe] border-t-[#7c3aed]"
                ></span>
            </div>

            <div>
                <strong class="block text-[13px] text-[#172033]">
                    Analyzing your question
                </strong>

                <p class="mt-0.5 text-[11px] text-[#98a2b3]">
                    Planning the analysis and preparing your results...
                </p>
            </div>
        </div>
        ...
        <div
            v-if="error"
            class="mt-[18px] flex items-start gap-[13px] rounded-[14px] border border-red-200 bg-[#fffafa] px-[18px] py-4"
        >
            <div
            ...
            <div class="min-w-0">
                ...
                <button
                    ...
                    @click="analyze"
                    :disabled="loading || !question.trim()"
                    class="mt-3 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Try Again
                </button>
            </div>
        ...
        <div
            v-if="result"
            class="mt-6 rounded-[20px] border border-[#e7e5f2] bg-white p-6 shadow-[0_5px_18px_rgba(15,23,42,0.04)] max-[700px]:mt-[18px] max-[700px]:p-4"
        >
            <!-- Result Header -->
            ...
            <div
                ...
                "
                class="mt-4 w-full rounded-[14px] border border-[#edf0f5] bg-[#fcfcfe] p-[18px]"
            >
            ...
            <div
                ...
                class="mt-4 overflow-hidden rounded-[14px] border border-[#e8eaf1] bg-white"
            >
                <!-- Table Header -->
                ...
                <div class="w-full overflow-x-auto">
                    <table
                        class="min-w-[600px] w-full border-collapse"
                    >
                        <thead>
                            <tr>
                                <th
                        ...
                        <tbody>
                            ...
                            <tr
                                v-if="!result.data?.rows?.length"
                            >
                                <td
                                    :colspan="
                                        result.data?.columns?.length || 1
                                    "
                                    class="px-4 py-8 text-center text-xs text-[#98a2b3]"
                                >
            ...
            <div
                ...
                "
                class="rounded-[14px] border border-[#edf0f5] bg-[#fafbfc] px-5 py-8 text-center"
            >
                <div class="text-2xl text-[#c4b5fd]">✦</div>

                <p class="mt-2 text-sm font-semibold text-[#475467]">
                    Analysis completed
                </p>

                <p class="mt-1 text-xs text-[#98a2b3]">
                    The analysis did not return visualizable results.
                </p>
            </div>
        </div>
    </section>
